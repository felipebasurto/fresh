...
The Session worker creates one active `FacetHost` for its built-in providers and `SessionWorkerRuntime.facets` with `createFacetHost()` from `../facets.ts`. The host exposes the services provided by those facets and owns their lifecycle. The service-only TUI activates its server and Session facets through `activateRemoteFacets()`, which returns a disposable `RemoteFacetActivation`. Hosts bind their concrete local services before activation, and facets declare every local or remote dependency through the same `env.use()` operation. Synchronous setup-time `env.provide()`, `env.provideMany()`, `env.use()`, and `env.observe()` calls produce the internal dependency graph; setup does not repeat a declarative dependency list, and service handles remain disconnected until the complete graph validates. Providers activate before consumers, observations connect with their consuming facet, and disposal runs in reverse dependency order for the complete generation.
