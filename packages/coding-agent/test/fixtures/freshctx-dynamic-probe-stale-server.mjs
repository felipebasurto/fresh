// Hermetic FreshCtx probe fixture. Same protocol as freshctx-fake-server.mjs,
// except after the first successful commit every prepare drops TARGET_RATE from
// the projection while leaving other current lines. Used only by the offline
// dynamic-context probe to prove WRONG blocks provider HTTP.
      ...
      case "prepare": {
        ...
        if (budget > 0) {
          ...
          for (const unitId of selected) {
            ...
            // After the first successful commit, deliberately project an
            // unrelated current region that drops the observed TARGET_RATE
            // referent while it still survives on disk → Fresh oracle WRONG.
            if (commitsApplied > 0) {
              const dropped = bytes
                .toString("utf-8")
                .split("\n")
                .filter((line) => !line.includes("TARGET_RATE"))
                .join("\n");
              parts.push(`--- ${unit.path} ---\n${dropped.length > 0 ? dropped : "# unrelated\n"}`);
            } else {
