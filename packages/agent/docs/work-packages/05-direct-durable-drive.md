...
M11 first adds a deterministic repository fixture and measurement script; the external approximately 300 KB mini Session is motivation, not a checked-in benchmark. It must measure frame count and encoded-byte growth across Memory, JSONL, and SQLite, then bound amplification without weakening the existing crash contract: an admitted assistant effect remains reconstructible, progress observation remains useful, and settlement still removes operation-owned pending-frame state. Generic JSONL current-state snapshot compaction is a separate missing §1.7 backend mechanism: it reclaims all dead write history after settlement, while M11 bounds frame progress during an open effect. Their metrics overlap, so Memory/SQLite M11 work may proceed independently but the final JSONL post-settlement budget must include snapshot compaction. Investigate coalesced durable checkpoints, bounded frame snapshots plus a short delta tail, and JSONL snapshot-compaction interaction. Do not solve this by dropping unknown-outcome recovery, persisting only the final response, or adding a second generic retention framework.
