...
function outcomeFromFinalizedCall(finalized: FinalizedToolCall): ToolOutcome {
	return { message: createToolResultMessage(finalized), terminate: finalized.terminate };
}
	...
	await lane.settleOperation(
		...
		async (_state, run, _meta, reader) => {
			...
			return {
				kind: "commit",
				writes: [
					setValue(pendingEntry(call.resultEntryId), { type: "message", payload: message }),
...
async function performToolInvocation<TContext extends object | undefined>(
	...
	try {
		...
		return {
			message: recovery ? interruptedMessage(cleared.toolCall, undefined) : abortedMessage(cleared.toolCall),
			terminate: false,
		};
	}
	...
	const cancelled = lane.state.operation!.state.control.status === "cancel_requested";
	...
	return { message: createToolResultMessage(finalized), terminate: cancelled ? false : finalized.terminate };
}
...
async function prepareToolInvocation<TContext extends object | undefined>(
	...
	if (sources.assistant.stopReason === "length") {
		return { kind: "outcome", outcome: { message: truncatedMessage(toolCall), terminate: false } };
	}
	...
	try {
		...
		if (!(error instanceof AbortRequested)) throw error;
		await error.cancellation;
		return { kind: "outcome", outcome: { message: abortedMessage(toolCall), terminate: false } };
	}
...
async function startToolInvocation<TContext extends object | undefined>(
	...
	if (prepared.kind === "outcome") {
		return {
			completion: publishToolOutcome(lane, drive, run, call, prepared.outcome.message, prepared.outcome.terminate),
		};
	}
	...
	return {
		completion:
			effectPending.kind === "cancel_requested"
				? publishToolOutcome(lane, drive, run, call, abortedMessage(prepared.cleared.toolCall), false)
				: performToolInvocation(
						...
						publishToolOutcome(lane, drive, run, effectPending.value, outcome.message, outcome.terminate),
					),
	};
}
...
async function recoverToolInvocation<TContext extends object | undefined>(
	...
	if (!cancelled && call.replay === "safe" && tool?.replay === "safe") {
		const args = await clearReplayCheckpoint(lane, drive, run.batch, call);
		...
		return {
			completion: performToolInvocation(lane, drive, run.batch, call, cleared, toolContext, true).then((outcome) =>
				publishToolOutcome(lane, drive, run, call, outcome.message, outcome.terminate),
			),
		};
	}
	const checkpoint = await readCheckpoint(lane, drive, call);
	return {
		completion: publishToolOutcome(lane, drive, run, call, interruptedMessage(toolCall, checkpoint), false),
	};
}
	...
	for (let transition = 0; transition <= batch.calls.length * 2 + 1; transition += 1) {
		...
		if (current.run.control.status === "cancel_requested") {
			...
			if (call.status === "planned") {
				await publishToolOutcome(lane, drive, current.run, call, abortedMessage(toolCall), false);
			} else {
				const checkpoint = await readCheckpoint(lane, drive, call);
				await publishToolOutcome(lane, drive, current.run, call, interruptedMessage(toolCall, checkpoint), false);
			}
			continue;
		}
