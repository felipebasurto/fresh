...
describe("tracker: intent", () => {
	...
	it("round-trips interleaved writes", () => {
		...
		const ops = t.flush();
		expect(apply(structuredClone(initial), ops)).toEqual(t.target);
	});
	...
	it("invalidates a pending child when its parent is overwritten", () => {
		...
		t.state.a.b = 99;
		t.state.a = { c: 2 };
		expect(apply(structuredClone(initial), t.flush())).toEqual(t.target);
	});
});
...
describe("tracker: root ops", () => {
	...
	it("matches splice with no arguments", () => {
		...
		expect(t.target).toEqual(initial);
		expect(apply(structuredClone(initial), t.flush())).toEqual(t.target);
	});
	...
	it("detaches objects supplied to fill", () => {
		...
		supplied.value = 2;
		expect(t.target.xs).toEqual([{ value: 1 }, { value: 1 }]);
		expect(t.target.xs[0]).not.toBe(t.target.xs[1]);
		expect(apply({ xs: [{ value: 0 }, { value: 0 }] }, t.flush())).toEqual(t.target);
	});
	...
	it("keeps copyWithin results as a JSON tree", () => {
		...
		expect(t.target.xs).toEqual([{ value: 1 }, { value: 1 }]);
		expect(t.target.xs[0]).not.toBe(t.target.xs[1]);
		expect(apply({ xs: [{ value: 1 }, { value: 2 }] }, t.flush())).toEqual(t.target);
	});
	...
	it("grows arrays with explicit null values", () => {
		...
		t.flush();
		t.state.xs.length = 4;
		expect(t.target.xs).toEqual([1, null, null, null]);
		expect(apply(structuredClone(initial), t.flush())).toEqual(t.target);
	});
});
...
describe("replacing the whole value", () => {
	...
	it("rejects assigning a nested tracked proxy into state", () => {
		...
		}).toThrow(/tracked prox/);
		expect(() => {
			t.state.wrapped = { inner: t.state.left };
		}).toThrow(/tracked prox/);
		t.state.items = [];
		expect(() => t.state.items?.push(t.state.left)).toThrow(/tracked prox/);
...
describe("the first flush", () => {
	...
	it("discard clears pending deltas", () => {
		const t = track({ x: 0 });
		t.flush();
		t.state.x = 1;
		expect(t.dirty).toBe(true);
...
describe("apply and fan-out", () => {
	...
	it("does not preserve aliases assigned to separate paths", () => {
		const t = track<{ a?: { value: number }; b?: { value: number } }>({});
		const replica = apply<{ a?: { value: number }; b?: { value: number } }>(undefined, t.flush());
		const shared = { value: 0 };
		t.state.a = shared;
		t.state.b = shared;
		t.state.a.value = 1;
		const next = apply(replica, t.flush());
		expect(next).toEqual(t.target);
		expect(t.target.b?.value).toBe(0);
	});
	...
	it("does not alias pushed values with an in-process consumer", () => {
		...
		const next = apply(replica, t.flush());
		next.xs[0]!.value = 2;
		expect(t.target.xs[0]!.value).toBe(1);
	});
...
describe("dead-op elimination", () => {
	...
	it("keeps the delete when a later append folds into the set", () => {
...
describe("flush", () => {
	...
	it("clones reserved value keys without invoking prototype setters", () => {
...
describe("safety: array indices", () => {
	...
	it("rejects sparse writes at the producer", () => {
		...
		expect(t.target.xs).toEqual([1, 2, 3]);
	});
